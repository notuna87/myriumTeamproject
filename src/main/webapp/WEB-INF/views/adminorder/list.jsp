		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">
					주문관리<span class="badge">관리자</span>
				</h1>
			</div>
		</div>
		<div class="row">
									...
									<tbody>
										<c:forEach var="entry" items="${groupedOrders}">
											...
											<tr>
												...
												<td class="text-center">${items[0].ordersId}<!-- NEW 라벨: 발송처리되지 않은 주문 --> <c:if test="${items[0].orderStatus == 1}">
														<c:out value="${items[0].orderStatus}"></c:out>
														<span class="badge badge-danger ml-1">NEW</span>
													</c:if></td>
												<td class="text-left">${items[0].productName} <c:if test="${fn:length(items) > 1}">
			                        외 ${fn:length(items) - 1}
			                      </c:if>
												</td>
												...
												<td class="text-center">
													<button class="btn btn-sm btn-primary" onclick="showOrderModal('${items[0].ordersId}')">자세히</button>
												</td>
											</tr>
										</c:forEach>
									</tbody>
				...
				.ready(
						function() {
							...
							window.showOrderModal = function(ordersId) {
								console.log(
										"showOrderModal called with ordersId:",
										ordersId);

								var modalBody = $('#modalBody');

								// 주문번호(ordersId) 기준 필터링
								var filteredItems = ordersList.filter(function(
										item) {
								...
								var html = '<h5>주문번호: ' + ordersId + '</h5>'
										+ '<p>주문일: ' + orderInfo.orderDate
										+ '</p>' + '<p>고객명: '
										+ orderInfo.receiver + '</p>'
										+ '<p>주소: ' + orderInfo.address
										+ '</p>' + '<p>전화번호: '
								...
								filteredItems
										.forEach(function(item) {
											...
											switch (item.order_status_product) {
											case 0:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="1">배송준비중</button>';
												break;
											case 1:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="2">배송중</button>';
												break;
											case 2:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="3">배송완료</button>';
												break;
											case 3:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="18">구매확정</button>';
												break;
											case 4:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="12">교환승인</button>'
														+ '<button class="btn btn-sm btn-danger update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="13">교환거절</button>';
												break;
											case 5:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="2">배송중</button>';
												break;
											case 6:
												html += '<button class="btn btn-sm btn-danger update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="14">반품승인</button>'
														+ '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="15">반품거절</button>';
												break;
											case 7:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="11">환불완료</button>';
												break;
											case 8:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="16">취소승인</button>'
														+ '<button class="btn btn-sm btn-danger update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="17">취소거절</button>';
												break;
											case 9:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id + '" data-status="11">환불완료</button>';
												break;
											...
											case 14:
												...
												break;
											case 15:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id +'" data-status="14">반품승인</button>';
												break;
											case 16:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id +'" data-status="9">환불완료</button>';
												break;
											case 17:
												html += '<button class="btn btn-sm btn-success update-status-btn" data-oid="' + item.ordersId + '" data-pid="' + item.orders_product_id +'" data-status="16">취소승인</button>';
												break;
												...
												$
														.ajax({
															...
															success : function(
																...
																$statusTd
																		.text(getStatusText(orderStatus));
							...
							function getActionButtons(status, ordersId,
									ordersProductId) {
								var html = '';
								switch (status) {
								case 0:
							...
							$(".paginate_button a").on(
									...
									function(e) {
										...
										actionForm.find("input[name='id']")
												.remove(); //뒤로가기 후 기존 파라미터 누적문제 해결
							...
							$(document)
									.on(
											...
											function() {
												...
												window.location.href = "/adminmember/modify?id="
														+ id;
											});
