...
$(document).ready(function () {
  ...
  $("#uploadInputThumbnail").on("change", function (e) {
    ...
    if (selectedImgsThumbnail.length + files.length > maxFiles) {
    ...
    files.forEach(file => {
      if (checkExtension(file.name, file.size)) {
    	  selectedImgsThumbnail.push(file);
      }
    });
    
    if (selectedImgsThumbnail.length > 0) {
  ...
  $("#uploadInputDetail").on("change", function (e) {
    ...
    if (selectedImgsDetail.length + files.length > maxFiles) {
      alert("❗ 최대  " + maxFiles + "개까지 파일을 업로드할 수 있습니다.");
      $(this).val('');
      return;
    }
    ...
    files.forEach(file => {
      if (checkExtension(file.name, file.size)) {
    	  selectedImgsDetail.push(file);
      }
    });
    
    if (selectedImgsDetail.length > 0) {
  ...
  $("#uploadBtnThumbnail").on("click", function (e) {
    ...
    selectedImgsThumbnail.forEach(file => formData.append("uploadFile", file));
  ...
  $("#uploadBtnDetail").on("click", function (e) {
    ...
    const formData = new FormData();
    selectedImgsDetail.forEach(file => formData.append("uploadFile", file));
  ...
  // 미리보기 영역 업데이트
  function updateFileListUIT() {
	  
    const list = $("#uploadListThumbnail");
    list.empty();

    selectedImgsThumbnail.forEach((file, index) => {
  ...
  function updateFileListUID() {
	  
    const list = $("#uploadListDetail");
    list.empty();

    selectedImgsDetail.forEach((file, index) => {
      const li = $("<li>").addClass("list-group-item d-flex align-items-center justify-content-between");
      const content = $("<div>").addClass("d-flex align-items-center");
      ...
      const delBtn = $("<button type='button'>")
        ...
        .text("삭제")
        .on("click", function () {
          selectedImgsDetail.splice(index, 1); // 배열에서 제거
          updateFileListUID();             // UI 다시 그림
        });
  ...
  $("button[type='reset']").on("click", function() {
    ...
    selectedImgsThumbnail = [];       // 선택된 파일 배열 초기화
    selectedImgsDetail = [];       // 선택된 파일 배열 초기화
	  uploadCompletedThumbnail = false;  // 업로드 완료 상태 초기화
	  uploadCompletedDetail = false;  // 업로드 완료 상태 초기화
	  uploadedThumbnailList = [];    // 업로드된 파일 목록 초기화
	  uploadedDetailList = [];    // 업로드된 파일 목록 초기화
  ...
  $("form").on("submit", function (e) {
    ...
    if (selectedImgThubmnail.length > 0 && !uploadCompletedThumbnail) {
      alert("파일 업로드를 먼저 완료해주세요.");
      e.preventDefault();
      return;
    }
    if (selectedImgsDetail.length > 0 && !uploadCompletedDetail) {
      alert("파일 업로드를 먼저 완료해주세요.");
      e.preventDefault();
      return;
    }
