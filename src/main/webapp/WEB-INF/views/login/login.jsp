    ...
    <div class="login_wrap">
      ...
      <form class="login_form" method="post" action="${pageContext.request.contextPath}/login">
        ...
        <c:if test="${not empty error}">
          <p class="error_msg" style="color:red; margin-bottom:10px; font-size:13px;">${error}</p>
        </c:if>
        <c:if test="${not empty logout}">
          <p class="success_msg" style="color:blue; margin-bottom:10px; font-size:13px;">${logout}</p>
        </c:if>

        <div class="login_option">
          <label><input type="checkbox" name="remember-me" /> 아이디 저장</label>
          <span class="secure">
            <img src="${pageContext.request.contextPath}/resources/img/login/icon_lock.png" alt="보안" />
            보안접속
          </span>
        </div>
